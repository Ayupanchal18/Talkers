...
import { Mail, Lock, AlertCircle, Video, ArrowRight, Eye, EyeOff, User } from 'lucide-react-native';
...
export default function RegisterPage({ onNavigateToLogin }: RegisterPageProps) {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      console.warn('[Register] Error logged in store:', err);
    }
  };
        ...
        >
          ...
          <View style={tw`gap-5`}>
            ...
            <View style={tw`gap-2`}>
              ...
              <View style={tw`flex-row items-center bg-[#0c1220] border border-[#1e293b] rounded-xl px-4 h-12`}>
                <User size={16} color="#64748b" style={tw`mr-3`} />
                <TextInput
            ...
            >
              {isLoading ? (
                ...
                <View style={tw`flex-row items-center gap-2`}>
                  <Text style={tw`text-white font-bold text-sm tracking-wide`}>Create account</Text>
          ...
          <View style={tw`flex-row justify-center items-center mt-8 gap-1.5`}>
            ...
            <TouchableOpacity onPress={onNavigateToLogin} activeOpacity={0.7}>
              <Text style={tw`text-blue-400 text-sm font-bold`}>Sign in</Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
      </KeyboardAvoidingView>
